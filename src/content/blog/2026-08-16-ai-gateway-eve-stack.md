...
Since the first draft of this post, the agent has also been deployed: it's live on Vercel, and I drive it from the eve TUI in remote mode (`eve dev <deployment-url>`) over Vercel OIDC — the same HTTP flow, exercised against the production URL instead of localhost.
...
- **The agent repo is committed, pushed, and deployed.** The magerbot project lives in the private GitHub repo [github.com/mager/magerbot](https://github.com/mager/magerbot) — not Hearth — the reviewed working tree is clean, and `.env` remains gitignored and was never pushed. The agent has been verified end-to-end locally and remotely: the production deployment is live and reachable through the streaming HTTP flow and through `eve dev <deployment-url>` in remote mode. What's left is keeping the gbrain service supervised, setting budgets, and deciding the Buzz/Eve adapter path.
- **Production auth for arbitrary browsers is in.** `agent/channels/eve.ts` no longer ships the scaffold's `placeholderAuth()`; it's a real auth walk — `vercelOidc()` for the CLI and Vercel-to-Vercel calls, HTTP Basic (`routeBasic()`, password from `EVE_ROUTE_PASSWORD` at request time) for browsers, `localDev()` for the local TUI ([details](/blog/2026-06-18-vercel-eve/)). The production URL no longer 401s browsers without OIDC.
